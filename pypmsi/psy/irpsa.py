...
def irpsa(finess, annee : int, mois : int, path : str, typi : int = 1, tdiag : bool = True, filepath = "", n_rows = None) -> dict:
    """Découpage des RPSA ; 201x à 202x
    ...
    if filepath != "":
        ...
        file_in = path + '/' + pmsi_format_fullname(finess, annee, mois, 'psy', 'rpsa')

    df = pl.read_csv(file_in, has_header=False, skip_rows=0, new_columns=["l"], n_rows = n_rows)

    df = parse_pmsi_fwf(df, "psy", "rpsa", annee)
